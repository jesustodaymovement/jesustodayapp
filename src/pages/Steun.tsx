...
const Steun = () => {
  ...
  const trustPoints = [
    t('Veilig betalen via iDEAL, creditcard of PayPal'),
    t('Fiscaal aftrekbaar via onze ANBI-status'),
    t('Eenmalig of maandelijks, jij kiest'),
  ];
  ...
  const packages = [
    { amount: '€25', title: t('Honderd kaartjes met QR-code voor één ambassadeur') },
    { amount: '€250', title: t('Een verhaal volledig geproduceerd op video') },
    { amount: '€1.000', title: t('Eerste videografie-apparatuur voor een nieuw land') },
  ];

  return (
    <>
      <Helmet>
        <title>{t('Steun JesusToday, doneer aan een generatie vol hoop')}</title>
        <meta
          name="description"
          content={t('Doneer aan JesusToday en help persoonlijke verhalen over Jezus verspreiden. Veilig, eenmalig of maandelijks en fiscaal aftrekbaar via onze ANBI-status.')}
        />
        <meta property="og:title" content={t('Steun JesusToday, doneer vandaag')} />
        <meta
          property="og:description"
          content={t('Doneer aan JesusToday en help persoonlijke verhalen over Jezus verspreiden. Veilig en fiscaal aftrekbaar via onze ANBI-status.')}
        />
        ...
        <meta name="twitter:title" content={t('Steun JesusToday, doneer vandaag')} />
        <meta name="twitter:description" content={t('Doneer aan JesusToday en help persoonlijke verhalen over Jezus verspreiden.')} />
      </Helmet>
      ...
      <header className="absolute top-0 left-0 right-0 z-20 py-5">
        <div className="container mx-auto px-6 flex justify-center md:justify-start">
          <Link to="/" aria-label={t('Naar de homepage van JesusToday')}>
              ...
              <div>
                <ScrollReveal>
                  <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gold/20 border border-gold/30 mb-6">
                    ...
                    <span className="text-anthracite text-sm font-medium">{t('Doneer aan JesusToday')}</span>
                  </div>
                </ScrollReveal>
                <ScrollReveal delay={100}>
                  <h1 className="text-4xl md:text-5xl font-bold text-anthracite leading-tight mb-6">
                    {t('Jouw gift laat verhalen')} <span className="text-gold">{t('landen.')}</span>
                  </h1>
                </ScrollReveal>
                <ScrollReveal delay={200}>
                  <p className="text-lg text-muted-foreground leading-relaxed mb-8">
                    {t('Help mee bouwen aan een beweging die een hele generatie bereikt. Met jouw donatie landen persoonlijke verhalen over Jezus op straat, werk en school, daar waar deze generatie zich beweegt.')}
                  </p>
                </ScrollReveal>
              ...
              <ScrollReveal delay={200}>
                <div className="rounded-3xl bg-white border border-anthracite/10 shadow-lg p-4 md:p-6">
                  <div className="flex items-center justify-center gap-2 mb-4">
                    ...
                    <span className="text-anthracite text-sm font-medium">{t('Veilig doneren via Donorbox')}</span>
                  </div>
                  <DonorboxWidget />
                </div>
              </ScrollReveal>
            ...
            <div className="max-w-3xl mx-auto text-center">
              ...
              <ScrollReveal delay={100}>
                <h2 className="text-2xl md:text-3xl font-bold text-anthracite mb-4">
                  {t('Transparant, betrouwbaar en')} <span className="text-gold">{t('fiscaal aftrekbaar.')}</span>
                </h2>
              </ScrollReveal>
              <ScrollReveal delay={200}>
                <p className="text-muted-foreground leading-relaxed mb-8">
                  {t('JesusToday is een initiatief van Stichting Cornerstone Ministries, een door de Belastingdienst erkende ANBI. Daardoor zijn je donaties onder voorwaarden aftrekbaar van de inkomstenbelasting.')}
                </p>
              </ScrollReveal>
              <ScrollReveal delay={300}>
                <div className="inline-flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-sm text-anthracite">
                  <span className="inline-flex items-center gap-2">
                    <ShieldCheck className="w-4 h-4 text-gold" /> {t('ANBI-stichting')}
                  </span>
                  <span className="inline-flex items-center gap-2">
